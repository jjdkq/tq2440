    ...
    ui(new Ui::MyDialog)
{
    ...
    char str[20];
    sprintf(str, "%04x-%02x-%02x", time.year, time.mon, time.date);
    ui->pushButton->setText(tr(str));

    sprintf(str, "%x", time.day);
    ui->pushButton_2->setText(tr(str));
...
void MyDialog::timerUpDate()
{
    ...
    qDebug("pressure = %ld  altitude = %.3f", pressure, altitude);
