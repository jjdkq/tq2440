...
u8 Ublox_Cfg_Ack_Check(void)
{
    u16 len=0,i;
    u8 rval=0;
    uart1_read(USART1_RX_BUF, USART1_MAX_RECV_LEN);
    len=strlen((char *)USART1_RX_BUF);              //此次接收到的数据长度
    for(i=0;i<len;i++)
    {
        if(USART1_RX_BUF[i]==0XB5)
            break;                          //查找同步字符 0XB5
    }
    if(i==len)
        rval=2;                             //没有找到同步字符
    else if(USART1_RX_BUF[i+3]==0X00)
        rval=3;                             //接收到NACK应答
    else
        rval=0;                             //接收到ACK应答
    //else
...
u8 Ublox_Cfg_Prt(u32 baudrate)
{
    ...
    delay_ms(200);				//等待发送完成
    uart1_init((char *)UART_NAME, 9600);           //重新初始化串口1
