...
#define DEVICE2_NAME "led2"		/*注册驱动时建立的设备名称*/ 
...
extern struct file_operations led2_fops;
static int __init led_init(void)
{
	int i=0,b=0;
//	i=register_chrdev(LED_MAJOR,DEVICE_NAME,&led_fops);	//注册字符设备(需要手动mknod led设备)
	b=register_chrdev(LED_MAJOR,DEVICE2_NAME,&led2_fops);	//注册字符设备(需要手动mknod led设备)
	i=misc_register(&misc);							//(自动)注册杂项设备(主设备号:10,自动分配次设备号)
	if(i<0||b<0)
		printk("led can't register!\n");
	else
		printk("myled1 & led2 inittialized\n");
	return i;
}
static void __exit led_exit(void)
{
	//unregister_chrdev(LED_MAJOR,DEVICE_NAME);				//卸载驱动
	unregister_chrdev(LED_MAJOR,DEVICE2_NAME);				//卸载驱动
	misc_deregister(&misc);						//卸载驱动
	printk ("my "DEVICE_NAME"&"DEVICE2_NAME" is over!\n");
}
