...
{   
	/* 从机(bmp180)的地址为:0xee >> 1(注意:在内核中i2c从机的读写控制位在从机     * 地址的最高位,而在裸机或者软件模拟i2c中,读写控制位在最低位);
	 * 从机名称为:bmp180(这个名称要与i2c_driver结构体中的名称(.driver.name)      * 一致,否则i2c_driver中的探测函数(.probe)不能被调用)
	 */
