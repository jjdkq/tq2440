...
void BMP_WriteOneByte(uint8_t WriteAddr,uint8_t DataToWrite)
{
	...
	i2c_send_one_byte(0xEE);
	i2c_wait_ack();
	
	i2c_send_one_byte(WriteAddr);
	i2c_wait_ack();
	
	i2c_send_one_byte(DataToWrite);
	i2c_wait_ack();
	myi2c_stop();
}
...
short BMP_ReadTwoByte(uint8_t ReadAddr)
{
	...
	i2c_send_one_byte(ReadAddr);
	i2c_wait_ack();
	
	myi2c_start();
	
	i2c_send_one_byte(0xEF);
	i2c_wait_ack();
	
	msb = i2c_read_one_byte(1);
	lsb = i2c_read_one_byte(0);
